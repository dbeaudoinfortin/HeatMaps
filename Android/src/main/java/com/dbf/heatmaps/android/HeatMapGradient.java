...
public class HeatMapGradient extends AbstractHeatMapGradient<Color>{
	...
	private static Color[] createHeatMapGradient(String[] colours) {
		...
		for(int i = 0; i < colours.length; i++) {
			colourArray[i] = Color.valueOf(Color.parseColor(colours[i]));
		}
		return colourArray;
	}
	...
	protected Color createColour(Color colour1, Color colour2, float fraction) {
		final float r = colour1.red() * (1 - fraction) + colour2.red() * fraction;
		final float g = colour1.green() * (1 - fraction) + colour2.green() * fraction;
		final float b = colour1.blue() * (1 - fraction) + colour2.blue() * fraction;
